...
public class OrdersFrame extends JFrame {
    ...
    private JPanel createOrderCard(ApiClient.OrderDTO order) {
        ...
        if ("PENDING".equals(order.getStatus()) || "CONFIRMED".equals(order.getStatus())) {
            ModernButton cancelBtn = new ModernButton("❌ Hủy đơn");
    ...
    private void cancelOrder(ApiClient.OrderDTO order) {
        int confirm = JOptionPane.showConfirmDialog(
            this,
            "Bạn có chắc chắn muốn hủy đơn hàng #" + order.getId() + "?",
            "Xác nhận hủy đơn",
        ...
        SwingWorker<ApiClient.CancelOrderResponse, Void> worker = new SwingWorker<>() {
            @Override
            protected ApiClient.CancelOrderResponse doInBackground() throws Exception {
                return ApiClient.getInstance().cancelOrder(order.getId());
            }
            
            @Override
            protected void done() {
                try {
                    ApiClient.CancelOrderResponse response = get();
                    ...
                    if (response.cancelledCount != null && response.cancelledCount >= 3) {
                    ...
                    if (errorMsg.contains("khóa")) {
                        ...
                        JOptionPane.showMessageDialog(
                            OrdersFrame.this,
                            "Không thể hủy đơn hàng: " + errorMsg,
                            "Lỗi",
                            JOptionPane.ERROR_MESSAGE
                        );
                    }
